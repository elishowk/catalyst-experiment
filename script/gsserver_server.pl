...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{GSSERVER_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{GSSERVER_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
gsserver_server.pl - Catalyst Testserver

=head1 SYNOPSIS

gsserver_server.pl [options]
